...
class rectangle(Sprite):
    def __init__(self, x=0, y=0, color=0xFF0000, width=20, height=20, scale=1, hidden=False):
        ...
        tile_grid = displayio.TileGrid(
            ...
            width=width, height=height, tile_width=1, tile_height=1
        )
    ...
    def width(self, width):
        ...
        tile_grid = displayio.TileGrid(
            self.bitmap, pixel_shader=self.palette,
            width=width, height=self.bitmap.height, tile_width=1, tile_height=1
        )
        tile_grid.hidden = hidden
        self.group.insert(0, tile_grid)
    ...
    def height(self, height):
        ...
        tile_grid = displayio.TileGrid(
            self.bitmap, pixel_shader=self.palette,
            width=self.bitmap.width, height=height, tile_width=1, tile_height=1
        )
        tile_grid.hidden = hidden
        self.group.insert(0, tile_grid)
...
class text(Sprite):
    def __init__(self, x=0, y=0, content="Hello", color=0xFFFFFF, hidden=False):
        super().__init__()
        self._label = label.Label(FONT, text=content, color=color)
...
def hide_all():
    """Masque tous les sprites."""
    for group in général:
        if len(group) > 0:
            group[0].hidden = True
