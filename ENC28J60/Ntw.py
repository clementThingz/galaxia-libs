...
_LOG_LEVEL_FATAL     = const(1)
_LOG_LEVEL_ERROR     = const(2)
_LOG_LEVEL_WARNING   = const(3)
_LOG_LEVEL_INFO      = const(4)
_LOG_LEVEL_DEBUG     = const(5)

_ETH_TYPE_IP4        = const(0x0800)
_ETH_TYPE_ARP        = const(0x0806)
_ETH_TYPE_8021Q      = const(0x8100)
_ETH_HDR_SIZE        = const(14)

_ETH_TYPE_IP4_BYTES  = bytes([_ETH_TYPE_IP4 >> 8, _ETH_TYPE_IP4 & 0xFF])
_ETH_ADDR_BCAST      = bytes([0xFF,0xFF,0xFF,0xFF,0xFF,0xFF])

_ARP_HEADER_LEN      = const(28)
_ARP_OP_REQUEST      = const(1)
_ARP_OP_REPLY        = const(2)

_IP4_TYPE_ICMP       = const(1)
_IP4_TYPE_TCP        = const(6)
_IP4_TYPE_UDP        = const(17)
_IP4_HDR_DF_FLAG     = const(0x40)
_IP4_HDR_MF_FLAG     = const(0x20)
_IP4_HDR_NOOPT_SIZE  = const(20)
_IP4_ADDR_BCAST      = bytes([255,255,255,255])
_IP4_ADDR_ZERO       = bytes([0,0,0,0])

_ICMP4_ECHO_REPLY    = const(0)
_ICMP4_UNREACHABLE   = const(3)
_ICMP4_ECHO_REQUEST  = const(8)

_UDP_HDR_SIZE        = const(8)

_TCP_HDR_NOOPT_FMT   = '!HHIIHHHH'
_TCP_HDR_NOOPT_SIZE  = const(20)
...
def procArp(pkt):
    ...
    if (_LOG_LEVEL_DEBUG <= pkt.ntw.logArpLevel):
        print(f'[ARP] Rx oper={oper}')

    if _ARP_OP_REQUEST == oper:
        if tpa == pkt.ntw.myIp4Addr:
            ...
            if 0 > n:
                if (_LOG_LEVEL_ERROR <= pkt.ntw.logArpLevel):
                    print(f'[ARP] Fail to send REPLY: error code {n}')
    elif _ARP_OP_REPLY == oper:
        if (_LOG_LEVEL_INFO <= pkt.ntw.logArpLevel):
...
def makeArpReply(eth_dst, eth_src, ip_src, ip_dst):
    ...
    rsp.append(eth_dst)
    rsp.append(eth_src)
    rsp.append(bytes([_ETH_TYPE_ARP >> 8, 0xFF & _ETH_TYPE_ARP, 0, 1, 8, 0, 6, 4, 0, _ARP_OP_REPLY]))
    rsp.append(eth_src)
    rsp.append(ip_src)
    rsp.append(eth_dst)
    rsp.append(ip_dst)
...
def makeArpRequest(eth_src, ip_src, ip_dst):
    rsp = []
    rsp.append(_ETH_ADDR_BCAST)
    rsp.append(eth_src)
    rsp.append(bytes([_ETH_TYPE_ARP >> 8, 0xFF & _ETH_TYPE_ARP, 0, 1, 8, 0, 6, 4, 0, _ARP_OP_REQUEST]))
    rsp.append(eth_src)
    rsp.append(ip_src)
    rsp.append(bytes(6))
    rsp.append(ip_dst)
...
def makeIp4Hdr(src, tgt, ident, prot, dataLen, flags=0, fragOffset=0, ttl=128, dscp=0, ecn=0):
    totlen = _IP4_HDR_NOOPT_SIZE + dataLen
    hdr = bytearray(_IP4_HDR_NOOPT_SIZE)
    hdr[0] = 0x45   # Version + IHL
    hdr[1] = ((dscp << 2) | (ecn & 0x03)) & 0xFF
    hdr[2] = (totlen >> 8 ) & 0xFF
    hdr[3] = totlen & 0xFF
    hdr[4] = (ident >> 8) & 0xFF
    hdr[5] = (ident) & 0xFF
    hdr[6] = (flags | ((fragOffset >> 8) & 0x1F)) & 0xFF
...
def sendIcmp4EchoReply(pkt):
    ...
    # IP
    ipHdr = makeIp4Hdr(pkt.ntw.myIp4Addr, pkt.ip_src_addr, pkt.ntw.ip4TxCount, _IP4_TYPE_ICMP, len(icmpRepl))
    pkt.ntw.ip4TxCount += 1

    # Eth
    rsp.append(pkt.eth_src)
    rsp.append(pkt.ntw.myMacAddr)
    rsp.append(_ETH_TYPE_IP4_BYTES)
...
def procIcmp4(pkt):
    ...
    if _ICMP4_ECHO_REQUEST == icmpType:
        sendIcmp4EchoReply(pkt)

    elif _ICMP4_UNREACHABLE == icmpType:
        if (_LOG_LEVEL_INFO <= pkt.ntw.logIcmp4Level):
            print(f'[ICMP4] Rx type={icmpType}(Destination Unreachable) code={pkt.frame[offset + 1]}')

    else:
        if (_LOG_LEVEL_DEBUG <= pkt.ntw.logIcmp4Level):
...
def procIp4(pkt):
    ...
    if 4 != pkt.ip_ver:
        if (_LOG_LEVEL_WARNING <= pkt.ntw.logIp4Level):
            print(f'[IP4] Rx version={pkt.ip_ver} not supported!')
        return

    if _IP4_HDR_NOOPT_SIZE != pkt.ip_hdrlen:
        if (_LOG_LEVEL_WARNING <= pkt.ntw.logIp4Level):
    ...
    if pkt.ip_dst_addr == pkt.ntw.myIp4Addr:
        if (_LOG_LEVEL_DEBUG <= pkt.ntw.logIp4Level):
            print(f'[IP4] Rx my address protocol={pkt.ip_proto}')

        if _IP4_TYPE_ICMP == pkt.ip_proto:
            procIcmp4(pkt)
        elif _IP4_TYPE_TCP == pkt.ip_proto:
            procTcp4(pkt)
        elif _IP4_TYPE_UDP == pkt.ip_proto:
            procUdp4(pkt, bcast=False)

    elif pkt.ip_dst_addr == _IP4_ADDR_BCAST:
        if _IP4_TYPE_UDP == pkt.ip_proto:
...
def procEth(pkt):
    if (_LOG_LEVEL_DEBUG <= pkt.ntw.logEthLevel):
        printEthPkt(pkt)

    pkt.eth_dst = pkt.frame[0:6]
    pkt.eth_src = pkt.frame[6:12]
    pkt.eth_type, = struct.unpack_from("!H", pkt.frame, 12)
    pkt.eth_offset = _ETH_HDR_SIZE

    if _ETH_TYPE_8021Q == pkt.eth_type:
        pkt.eth_type, = struct.unpack_from("!H", pkt.frame, 14)
        pkt.eth_offset += 2

    if _ETH_TYPE_IP4 == pkt.eth_type:
        procIp4(pkt)
    elif _ETH_TYPE_ARP == pkt.eth_type:
        procArp(pkt)
    # ignore not supported types


def makeUdp4Hdr(srcIp, srcPort, dstIp, dstPort, data):
    udpLen = len(data) + _UDP_HDR_SIZE

    chksm = sum(struct.unpack('!HH', srcIp))
    chksm += sum(struct.unpack('!HH', dstIp))
    chksm += _IP4_TYPE_UDP + 2*udpLen + srcPort + dstPort
    chksm = calcChecksum(data, chksm)

    udpHdr = bytearray(_UDP_HDR_SIZE)
    udpHdr[0] = (srcPort >> 8) & 0xFF
    udpHdr[1] = (srcPort) & 0xFF
    udpHdr[2] = (dstPort >> 8) & 0xFF
    udpHdr[3] = (dstPort) & 0xFF
    udpHdr[4] = (udpLen >> 8) & 0xFF
    udpHdr[5] = (udpLen) & 0xFF
    udpHdr[6] = (chksm >> 8) & 0xFF
    udpHdr[7] = (chksm) & 0xFF
    return udpHdr
...
def procUdp4(pkt, bcast=False):
    ...
    pkt.udp_dataLen = udpLen - _UDP_HDR_SIZE
    pkt.udp_data = memoryview(pkt.frame[offset+_UDP_HDR_SIZE:offset+udpLen])
    ...
    if (0 != chksm_rx):
        ...
        chksm += _IP4_TYPE_UDP + 2*udpLen + pkt.udp_srcPort + pkt.udp_dstPort
        chksm = calcChecksum(pkt.udp_data, chksm)
        if 0 == chksm:
            chksm = 0xFFFF
        if (chksm != chksm_rx):
            if (_LOG_LEVEL_ERROR <= pkt.ntw.logUdp4Level):
...
def makeTcp4Hdr(srcIp, srcPort, dstIp, dstPort, data, tcp_seq_num, tcp_ack_num, flags, tcp_window_size, tcp_options_raw):
    tcpHdrLen = _TCP_HDR_NOOPT_SIZE + len(tcp_options_raw)
    do_flags = (0xF000 & (tcpHdrLen << 10)) | (0x1FF & flags)
    tcpLen = tcpHdrLen + len(data)

    chksm = sum(struct.unpack('!HH', srcIp))
    chksm += sum(struct.unpack('!HH', dstIp))
    chksm += _IP4_TYPE_TCP + tcpLen

    hdr = bytearray(struct.pack(_TCP_HDR_NOOPT_FMT, srcPort, dstPort, tcp_seq_num, tcp_ack_num, do_flags, tcp_window_size, 0, 0))
    hdr += tcp_options_raw
...
def procTcp4(pkt):
    offset = pkt.ip_offset
    pkt.tcp_srcPort, pkt.tcp_dstPort, pkt.tcp_seq_num, pkt.tcp_ack_num, do_flags, pkt.tcp_window_size, chksm_rx, pkt.tcp_urg_ptr = struct.unpack_from(_TCP_HDR_NOOPT_FMT, pkt.frame, offset)
    pkt.tcp_data_offset = do_flags >> 10
    pkt.tcp_flags = do_flags & 0x1FF
    pkt.tcp_options_raw = memoryview(pkt.frame[offset + _TCP_HDR_NOOPT_SIZE: offset + pkt.tcp_data_offset])
    ...
    if 0 != chksm:
        if (_LOG_LEVEL_ERROR <= pkt.ntw.logTcp4Level):
...
class Ntw:
    def __init__(self, nicSpi, nicCsPin):
        ...
        # Log level
        self.logEthLevel = _LOG_LEVEL_INFO
        self.logArpLevel = _LOG_LEVEL_INFO
        self.logIp4Level = _LOG_LEVEL_WARNING
        self.logIcmp4Level = _LOG_LEVEL_INFO
        self.logUdp4Level = _LOG_LEVEL_ERROR
        self.logTcp4Level = _LOG_LEVEL_ERROR

        self.nic.init()

        if (_LOG_LEVEL_INFO <= self.logEthLevel):
    ...
    def rxAllPkt(self):
        ...
        while True:
            ...
            ## unlock
            if 0 >= rxLen:
                if (_LOG_LEVEL_ERROR <= self.logEthLevel):
    ...
    def sendUdp4(self, tgt_ip, tgt_port, data, src_port=0):
        ...
        msg.append(_ETH_TYPE_IP4_BYTES)

        ip_totlen = _IP4_HDR_NOOPT_SIZE + _UDP_HDR_SIZE + data_len
        if ip_totlen <= self.getEthMTU():
            msg.append(makeIp4Hdr(self.myIp4Addr, tgt_ip, self.ip4TxCount, _IP4_TYPE_UDP, _UDP_HDR_SIZE + data_len))
            msg.append(makeUdp4Hdr(self.myIp4Addr, src_port, tgt_ip, tgt_port, data))
            msg.append(data)
            n = self.txPkt(msg)
        else:
            # IP fragmentation
            ip_mfo = ((self.getEthMTU() - _IP4_HDR_NOOPT_SIZE) >> 3) << 3

            n = 0
            first_frag = True
            data_frag_start = 0
            data_frag_stop = ip_mfo - _UDP_HDR_SIZE
            ip_frag_offset = 0
            ...
            while data_frag_start < data_len:
                ...
                msg.append(makeIp4Hdr(self.myIp4Addr,
                    ...
                    0 if last_frag else _IP4_HDR_MF_FLAG,
                    ip_frag_offset))
                if first_frag:
                    msg.append(makeUdp4Hdr(self.myIp4Addr, src_port, tgt_ip, tgt_port, data))
    ...
    def sendUdp4Bcast(self, tgt_port, src_port, data, src_ip4Addr=None):
        msg = []
        tgt_ip4Addr = _IP4_ADDR_BCAST
        if src_ip4Addr is None:
            src_ip4Addr = _IP4_ADDR_ZERO
        msg.append(_ETH_ADDR_BCAST)
        msg.append(self.myMacAddr)
        msg.append(_ETH_TYPE_IP4_BYTES)
        msg.append(makeIp4Hdr(src_ip4Addr, tgt_ip4Addr, self.ip4TxCount, _IP4_TYPE_UDP, _UDP_HDR_SIZE + len(data)))
        self.ip4TxCount += 1
    ...
    def sendTcp4(self, tgt_ip, tgt_port, src_port, data, tcp_seq_num, tcp_ack_num, flags, tcp_window_size = 1024, tcp_options_raw = bytes(0)):
        ...
        msg.append(makeIp4Hdr(self.myIp4Addr, tgt_ip, self.ip4TxCount, _IP4_TYPE_TCP, _TCP_HDR_NOOPT_SIZE + tcp_options_len + data_len))
...
class Udp4EchoServer:
    ...
    def __init__(self, ntw):
        self.ntw = ntw
        self.logLevel = _LOG_LEVEL_INFO

    def __call__(self, pkt):
        if (_LOG_LEVEL_INFO <= self.logLevel):
